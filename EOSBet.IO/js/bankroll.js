...
const EOSBetBankroll = {
  ...
  getContractDetails: async function (web3) {
    ...
    try {
      ...
      EOSBetBankroll.currentUserTokenSupply = await getStaticMappingFromBankroll(
        EOSBetBankroll.bankrollInstance.balanceOf,
        web3.eth.accounts[0]
      );
      // calculate value of tokens given these values
      EOSBetBankroll.currentUserTokenValue = EOSBetBankroll.currentTotalBankrollBalance
        .times(EOSBetBankroll.currentUserTokenSupply)
        .dividedBy(EOSBetBankroll.currentTotalTokenSupply);

      $('#current-tokens').text(web3.fromWei(EOSBetBankroll.currentUserTokenSupply, 'ether'));
      $('#current-tokens-value').text(
        isNaN(EOSBetBankroll.currentUserTokenValue) ? 0 : web3.fromWei(EOSBetBankroll.currentUserTokenValue, 'ether')
      );

      if (EOSBetBankroll.currentUserTokenSupply.greaterThan(0)) {
        var mandatoryWaitTime = parseInt(
          await getStaticValueFromBankroll(EOSBetBankroll.bankrollInstance.WAITTIMEUNTILWITHDRAWORTRANSFER),
          10
        );
        var usersContributionTime = parseInt(
          await getStaticMappingFromBankroll(
            EOSBetBankroll.bankrollInstance.checkWhenContributorCanTransferOrWithdraw,
            web3.eth.accounts[0]
          ),
          10
        );
        ...
        if (usersContributionTime + mandatoryWaitTime >= thisTime) {
          ...
          $('#withdraw-info').append(
            inMinutes.toString() + ' minutes until you may cash in your tokens for ether, or transfer your tokens to a different account.'
          );
        }
      }

      // now get the deposit details
      EOSBetBankroll.maximumBankrollContributions = await getStaticValueFromBankroll(
        EOSBetBankroll.bankrollInstance.MAXIMUMINVESTMENTSALLOWED
      );
  ...
  deposit: function () {
    ...
    web3.eth.sendTransaction(
      { to: EOSBetBankroll.bankrollInstance.address, from: web3.eth.accounts[0], value: web3.toWei(depositAmt, 'ether') },
          ...
          if (txReceipt.logs.length === 0) {
            ...
            $('#deposit-info').html(
              'You have successfully deposited ' +
                web3.fromWei(amountEther, 'ether') +
                ' ether to our bankroll and have been given ' +
                web3.fromWei(amountTokens, 'ether') +
                ' EOSBet Stake Tokens! Thank you for contributing to the EOSBet Bankroll.'
            );
          }
        }
      }
  ...
  withdraw: function (all) {
    if (all === true) {
      ...
      EOSBetBankroll.bankrollInstance.cashoutEOSBetStakeTokens(
        web3.toWei(withdrawAmt, 'ether'),
  ...
  parseWithdrawLogs: async function (txHash) {
    ...
    if (txReceipt.logs.length === 0) {
      ...
      $('#withdraw-info').html(
        'You successfully cashed in ' +
          web3.fromWei(amountTokens, 'ether') +
          ' EOSBet Stake tokens and have been sent ' +
          web3.fromWei(amountEther, 'ether') +
          ' ether. Thank you for contributing to our bankroll!'
      );
    }
  }
};
